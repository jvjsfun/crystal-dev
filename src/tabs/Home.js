...
export default class Home extends Component {
  ...
  constructor() {
      ...
      this.state = {
          ...
          navParameters:['planName','dateName','dateEpoch', 'location', 'currentUser','friendInfo','invitedFriends','groupName','invitedFriendNames','invitedGroupNames','invitedGroups','selectionType','timeType','timeString',],
          currentUser:{},
          friendInfo:[],
            ...
            <View style={[styles.submit,{marginTop:0}]} >
                <ScrollView keyboardShouldPersistTaps = {"always"} scrollEnabled = {this.shouldBeScrollable(this._navPropOrDefault('planName','I want to...'))} horizontal showsHorizontalScrollIndicator = {false} contentContainerStyle = {{alignItems:'center'}}>
                  <TouchableHighlight style = {{width: this.getTouchableWidth(this._navPropOrDefault('planName','I want to...'))}} underlayColor="rgba(255,255,255,0.95)" onPress = {()=>this.goToPlanName()}>
                    <Text numberOfLines = {1} style={styles.submitText, (this.props.navigation.state.params && this.props.navigation.state.params.planName || defaultPlanName) ? styles.submitTextOverride : styles.submitText}>
                      {this._navPropOrDefault('planName',defaultPlanName)}
                    </Text>
                  </TouchableHighlight>
                </ScrollView>
            </View>
            ...
            <ScrollView keyboardShouldPersistTaps = {"always"} scrollEnabled = {this.shouldBeScrollable(this._navPropOrDefault('location','Where') )} horizontal showsHorizontalScrollIndicator = {false} contentContainerStyle = {{alignItems:'center'}}>
              <TouchableHighlight 
                ...
                <Text numberOfLines = {1} style={styles.timeplaceText}>
                  {this._navPropOrDefault('location', 'Wherever')}
                </Text>
              </TouchableHighlight>
            </ScrollView>
          </View>
  ...
  //Currently, I pass things from screen to screen using navparameters.
  //If there is a better way to do this, please let me know, using a HACKFIX comment.
  ...
  _planCanBeMade(){
    return this._navPropOrDefault('planName',defaultPlanName);
  }
  ...
  _getPlan(){
    ...
    plan.planName = this._navPropOrDefault('planName',defaultPlanName);
  ...
  _uploadPlan(plan){
    ...
    if(plan.timeType && plan.timeType === "vague" && (plan.timeString === "Whenever" || plan.timeString === "Now") ){
      //Handle the whenever or Now case.
    }
    else{
